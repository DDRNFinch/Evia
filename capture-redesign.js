    ...
    const render=()=>{
      $("#screen").innerHTML=
        '<div class="capture-page">'+
          '<div class="capture-head"><button class="capture-back" id="capture-back" aria-label="Back">‹</button><div class="capture-head-copy"><div class="capture-eyebrow">EVIDENCE PACK</div><h2>'+esc2(u[0])+'</h2></div></div>'+
          '<section class="capture-job-card">'+
            '<p class="capture-intro">Capture the whole job in one pack. Take photos from the beginning, middle and end of the job.</p>'+
          ...
          '</section>'+
          '<section class="capture-explain-card">'+
            '<div class="capture-label">EXPLAIN THE PROCESS AND WHAT YOU DID</div>'+
            '<div class="capture-label capture-mention-label">THINGS TO MENTION</div><div class="capture-pills mention">'+g.mention.map(x=>'<span>'+esc2(x)+'</span>').join("")+'</div>'+
            '<textarea class="capture-note" id="capture-note" placeholder="Write about the process and what you did…"></textarea>'+
