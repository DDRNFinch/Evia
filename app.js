...
async function openSupportingEvidence(){const behaviours=supportingBehaviours();$("#page-title").textContent="Supporting Evidence";$("#screen").innerHTML='<button class="secondary" id="back-supporting-course" type="button">‹ Back to course</button><div class="card"><div class="section-title">PORTFOLIO</div><h2>Supporting Evidence</h2><p>Add photos, videos, audio or documents that support a behaviour. Evia stores them separately from your normal course evidence.</p></div><div id="supporting-behaviour-list">'+behaviours.map((b,i)=>'<button type="button" class="card supporting-behaviour" data-supporting-behaviour="'+i+'"><div><div class="section-title">'+esc(b.code)+'</div><h3>'+esc(b.title)+'</h3><p>'+b.ksbs.length+' linked Skills/Knowledge</p></div><span>›</span></button>').join("")+'</div>';$("#back-supporting-course").onclick=()=>nav("course");document.querySelectorAll("[data-supporting-behaviour]").forEach(btn=>btn.onclick=()=>supportingCapture(behaviours[Number(btn.dataset.supportingBehaviour)]))}
