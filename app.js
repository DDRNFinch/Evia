...
function supportingCardIcon(type){const icons={photo:'<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="6.5" width="18" height="14" rx="3"></rect><path d="M8 6.5l1.4-2h5.2l1.4 2"></path><circle cx="12" cy="13.5" r="3.5"></circle></svg>',video:'<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="6.5" width="13" height="11" rx="2.5"></rect><path d="M16 10l5-3v10l-5-3z"></path></svg>',audio:'<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="8" y="3" width="8" height="12" rx="4"></rect><path d="M5 11.5a7 7 0 0 0 14 0M12 18.5V22M9 22h6"></path></svg>',document:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 2.5h8l4 4v15H6z"></path><path d="M14 2.5v5h4M9 12h6M9 16h6"></path></svg>'};return '<span class="supporting-type-icon">'+(icons[type]||icons.document)+'</span>'}
async function openSupportingEvidence(){
 const base={course};
 $("#page-title").textContent="Supporting Evidence";
 $("#screen").innerHTML=`<button class="secondary" id="back-supporting-course" type="button">‹ Back to course</button><div class="card"><div class="section-title">PORTFOLIO</div><h2>Supporting Evidence</h2><p>Anything else you want to keep as evidence for this course.</p></div><div class="supporting-list"><button type="button" class="card supporting-type" data-supporting-type="photo">${supportingCardIcon("photo")}<span class="supporting-type-copy"><strong>Take a photo</strong></span></button><button type="button" class="card supporting-type" data-supporting-type="video">${supportingCardIcon("video")}<span class="supporting-type-copy"><strong>Record a video</strong></span></button><button type="button" class="card supporting-type" data-supporting-type="audio">${supportingCardIcon("audio")}<span class="supporting-type-copy"><strong>Record Audio</strong></span></button><button type="button" class="card supporting-type" data-supporting-type="document">${supportingCardIcon("document")}<span class="supporting-type-copy"><strong>Upload a file</strong></span></button></div>`;
...
function chat(){
 const eviaName=()=>{try{return String(JSON.parse(localStorage.getItem("evia7-profile")||"{}").eviaName||"Evia").trim()||"Evia"}catch(_){return"Evia"}};
 ...
 $("#modal-root").innerHTML='<div class="overlay"><section class="sheet chat-sheet"><div class="sheet-head"><div><div class="chat-kicker"><span id="evia-chat-name">Evia</span></div><h2>What would you like to do?</h2></div><button class="close" id="x" aria-label="Close">×</button></div><div class="chat" id="chat"><div class="bubble evia">I can help you review your learning, evidence and confidence.</div><div class="chat-options">'+options.map((o,i)=>'<button class="chat-pill" data-chat-option="'+i+'"><strong>'+o[0]+'</strong></button>').join("")+'</div></div></section></div>';
 const chatName=document.getElementById("evia-chat-name");if(chatName)chatName.textContent=eviaName();$("#x").onclick=()=>{ $("#modal-root").innerHTML=""; fab.classList.remove("chat-active"); const profileBtn=$("#profile-btn"); if(profileBtn && ["learning","course","progress","portfolio"].includes(screen))profileBtn.style.display="flex"; };
