...
(function(){
  ...
  function previewSample(draft,key){
    const guideStyle=key==="readingGuide"?' style="--preview-guide-position:'+(Number(draft.readingGuidePosition??48))+'%;--preview-guide-background:'+(({clear:"transparent",yellow:"rgba(255,220,0,.55)",blue:"rgba(80,160,255,.42)",pink:"rgba(255,100,160,.38)"})[draft.readingGuideColour||"yellow"])+'"':"";return '<div class="settings-preview-card '+previewClass(draft,key)+'"'+guideStyle+'><span class="preview-eyebrow">EVIA</span><h3>Read this example</h3><p>Making a change here lets you see exactly how Evia will look when this accessibility setting is applied.</p><button type="button">Example button</button></div>';
  }
