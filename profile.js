...
(function(){
  ...
  function applySettings(s){const root=document.documentElement;root.style.setProperty("--evia-text-scale",(Number(s.textScale||100)/100).toFixed(2));root.classList.remove("evia-text-scale-115","evia-text-scale-130","evia-text-scale-150");if(["115","130","150"].includes(String(s.textScale)))root.classList.add("evia-text-scale-"+s.textScale);root.classList.toggle("evia-dyslexia-font",!!s.dyslexiaFont);root.classList.toggle("evia-letter-spacing",!!s.letterSpacing);root.classList.toggle("evia-line-spacing",!!s.lineSpacing);root.classList.toggle("evia-reading-guide",!!s.readingGuide);root.classList.toggle("evia-focus-mode",!!s.focusMode);root.classList.toggle("evia-high-contrast",!!s.highContrast);root.classList.toggle("evia-reduced-motion",!!s.reducedMotion);root.dataset.eviaOverlay=s.colourOverlay||"none"}
  function openSettings(){
    const saved=getSettings(),draft=Object.assign({},saved);
    ...
    const restore=()=>applySettings(saved);
    const preview=()=>{applySettings(draft);document.querySelectorAll("[data-text-size]").forEach(b=>b.classList.toggle("selected",b.dataset.textSize===draft.textScale));document.querySelectorAll("[data-overlay]").forEach(b=>b.classList.toggle("selected",b.dataset.overlay===draft.colourOverlay));document.querySelectorAll("[data-setting]").forEach(i=>i.checked=!!draft[i.dataset.setting]);const a=document.querySelector(".preview-after");if(a)a.className="settings-preview-card preview-after "+(draft.dyslexiaFont?"preview-dyslexia ":"")+(draft.letterSpacing?"preview-letter ":"")+(draft.lineSpacing?"preview-lines ":"")+(draft.highContrast?"preview-contrast ":"")+(draft.colourOverlay!=="none"?"preview-overlay-"+draft.colourOverlay:"")};
    document.getElementById("modal-root").innerHTML=
      '<div class="profile-overlay settings-overlay"><section class="profile-sheet settings-sheet"><div class="profile-head"><div><div class="profile-kicker">ACCESSIBILITY & SETTINGS</div><h2>Make Evia work for you</h2></div><button class="profile-close" id="settings-close" aria-label="Close">×</button></div><p class="settings-intro">Try a change first. The preview shows the difference before you save it.</p>'+
      '<div class="settings-preview"><div class="settings-preview-head"><strong>BEFORE</strong><strong>AFTER</strong></div><div class="settings-preview-grid"><div class="settings-preview-card"><span class="preview-eyebrow">EVIA</span><h3>Read this example</h3><p>This is how your text looks now.</p><button type="button">Example button</button></div><div class="settings-preview-card preview-after"><span class="preview-eyebrow">EVIA</span><h3>Read this example</h3><p>This is how your text will look with your selected settings.</p><button type="button">Example button</button></div></div></div>'+
      '<div class="settings-section"><div class="settings-label">TEXT SIZE</div><div class="settings-segment">'+["100","115","130","150"].map(v=>'<button type="button" data-text-size="'+v+'" class="'+(draft.textScale===v?"selected":"")+'">'+v+'%</button>').join("")+'</div></div>'+
      '<div class="settings-section"><div class="settings-label">READABILITY</div>'+opts.map(o=>'<label class="setting-toggle"><span><strong>'+o[1]+'</strong><small>'+o[2]+'</small></span><input type="checkbox" data-setting="'+o[0]+'" '+(draft[o[0]]?"checked":"")+'><i aria-hidden="true"></i></label>').join("")+'</div>'+
      '<div class="settings-section"><div class="settings-label">COLOUR OVERLAY</div><div class="overlay-options">'+[["none","None"],["cream","Cream"],["soft-yellow","Soft yellow"],["soft-blue","Soft blue"],["soft-pink","Soft pink"]].map(o=>'<button type="button" data-overlay="'+o[0]+'" class="'+(draft.colourOverlay===o[0]?"selected":"")+'"><i></i><span>'+o[1]+'</span></button>').join("")+'</div></div>'+
      '<div class="settings-actions"><button type="button" class="secondary" id="settings-cancel">Cancel</button><button type="button" class="primary" id="settings-save">Confirm changes</button></div><div class="settings-section settings-about"><div class="settings-label">APPLIES ACROSS EVIA</div><p>Your accessibility choices apply to the whole app, including learning, course, progress, portfolio, profile, settings and Evia content.</p></div></section></div>';
    document.getElementById("settings-close").onclick=()=>{restore();openProfile()};
    document.getElementById("settings-cancel").onclick=()=>{restore();openProfile()};
    document.getElementById("settings-save").onclick=()=>{saveSettings(draft);openSettings()};
    document.querySelectorAll("[data-text-size]").forEach(b=>b.onclick=()=>{draft.textScale=b.dataset.textSize;preview()});
    document.querySelectorAll("[data-setting]").forEach(i=>i.onchange=()=>{draft[i.dataset.setting]=i.checked;preview()});
    document.querySelectorAll("[data-overlay]").forEach(b=>b.onclick=()=>{draft.colourOverlay=b.dataset.overlay;preview()});
  }
