...
const RUNTIME_SCRIPTS=[
  ...
  './evia-approved-ux-cleanup-v3.js?v=1',
  './nisia-loader.js'
];
  ...
  e.waitUntil((async()=>{
    ...
    if(internalReload){
      ...
      await Promise.all(windows.map(client=>{
        try{const url=new URL(client.url);url.searchParams.set('__evia_refresh','43');return client.navigate(url.href).catch(()=>null)}catch{return null}
      }));
    }
  })());
});
