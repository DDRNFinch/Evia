  ...
  e.waitUntil((async()=>{
    ...
    if(internalReload){
      ...
      await Promise.all(windows.map(client=>{
        try{const url=new URL(client.url);url.searchParams.set('__evia_refresh','56');return client.navigate(url.href).catch(()=>null)}catch{return null}
      }));
    }
  })());
});
