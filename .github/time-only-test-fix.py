...
HARNESS = r'''<!doctype html><html><head></head><body>
...
test('Time has one source of truth and reuses the existing progress calculations', async () => {
  ...
  expect(worker).toContain("const C='evia-pwa-v85'");expect(worker).toContain("url.searchParams.set('__evia_refresh','85')");
});
''', encoding='utf-8')
