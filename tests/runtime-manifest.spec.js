...
test('service worker uses the manifest as its runtime source and is v82', async () => {
  const worker = fs.readFileSync(path.join(root, 'service-worker.js'), 'utf8');
  expect(worker).toContain("importScripts('./evia-runtime-manifest.js')");
  expect(worker).toContain("const C='evia-pwa-v82'");
  expect(worker).toContain("url.searchParams.set('__evia_refresh','82')");
  expect(worker).not.toMatch(/const\s+RUNTIME_SCRIPTS\s*=\s*\[/);
});

test('EPA zone v2 loads last so it owns the approved EPA experience', async () => {
  ...
  expect(v2).toBeGreaterThan(demo);
