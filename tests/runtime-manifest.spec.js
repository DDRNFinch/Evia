...
test('service worker uses the manifest as its runtime source and is v80', async () => {
  const worker = fs.readFileSync(path.join(root, 'service-worker.js'), 'utf8');
  expect(worker).toContain("importScripts('./evia-runtime-manifest.js')");
  expect(worker).toContain("const C='evia-pwa-v80'");
  expect(worker).toContain("url.searchParams.set('__evia_refresh','80')");
