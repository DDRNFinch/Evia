...
function percentage(){
  return Math.round(Math.max(0,Math.min(1,currentMinutes()/MINIMUM_MINUTES))*100);
}
function patch(){
...
function observeRoot(){
  ...
  if(!root)return;
  new MutationObserver(()=>requestAnimationFrame(patch)).observe(root,{childList:true});
}
