(()=>{
"use strict";
const COURSE_ID="6570-05",MARKER="nisi-6570-smoke-v1";
const ROUTES={
...
function mapStats(path,allowed){
  const mapped=new Set(),ids=new Set(),unknown=new Set();let jobs=0,opportunities=0,empty=0;
  (path.siteData||[]).forEach(cat=>{(cat.jobs||[]).forEach(job=>{jobs++;(job.opps||[]).forEach(op=>{
    ...
    const codes=(op.codes||[]).map(String);if(!codes.length)empty++;codes.forEach(code=>{mapped.add(code);if(!allowed.has(code))unknown.add(code)})
  })})});
  return{categories:(path.siteData||[]).length,jobs,opportunities,mapped:mapped.size,empty,unknown:[...unknown]}
}
function run(){
  ...
  for(const [route,expected] of Object.entries(ROUTES)){
    ...
    if(stats.mapped!==expected.acs||stats.empty||stats.unknown.length)errors.push(`${route}: evidence map coverage`);
