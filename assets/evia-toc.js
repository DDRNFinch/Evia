...
const COURSES=[
  {id:"st0095-v1-2",title:"Bricklayer — ST0095 v1.2"}
];
...
function addMonthsClamped(dayMs,months){
  const d=new Date(dayMs),day=d.getUTCDate();
  const baseMonth=d.getUTCMonth()+months;
  const year=d.getUTCFullYear()+Math.floor(baseMonth/12);
...
function calendarDiff(fromMs,toMs){
  ...
  let anchor=addMonthsClamped(fromMs,months);
...
function formatSpan(span){
  const m=span.months,d=span.days;
  return `${m} month${m===1?"":"s"} · ${d} day${d===1?"":"s"}`;
}
function formatDate(value){
  const ms=parseDay(value);if(ms===null)return "Not set";
...
function coursePosition(t=currentTimeline()){
  ...
  const elapsedEnd=Math.min(Math.max(today,start),end);
  const remainingStart=Math.min(Math.max(today,start),end);
...
function patchName(){
  ...
  if(!name)return;
  document.querySelectorAll(".self-top small").forEach(el=>{if(el.textContent!==name)el.textContent=name});
}
...
function layer(body,title="My course",back=null){
  ...
  el.className="evia-tools-layer evia-toc-layer evia-toc";
  ...
  el.querySelector("[data-toc-back]").onclick=back||closeLayer;
  return el;
}
function summary(){
  const t=currentTimeline(),pos=coursePosition(t),name=currentName()||"Name not set";
  if(!pos.valid)return edit();
  const el=layer(`
    ...
    <div class="evia-toc-profile"><b>${esc(name)}</b><span>${esc(t.courseTitle)}</span></div>
...
function edit(){
  ...
  const options=COURSES.map(c=>`<option value="${esc(c.id)}" ${c.id===t.courseId?"selected":""}>${esc(c.title)}</option>`).join("");
  const el=layer(`
    <h2>Course details</h2>
    <p class="evia-tools-copy">These details set your time-on-course progress. Your full name is the same name Evia uses throughout the app.</p>
    <div class="evia-toc-form">
      ...
      <label>Course<select data-toc-course>${options}</select><small>More courses can be added to Evia later.</small></label>
  ...
  el.querySelector("[data-save-course]").onclick=()=>{
    const fullName=el.querySelector("[data-toc-name]").value.trim();
    const courseId=el.querySelector("[data-toc-course]").value;
    const startDate=el.querySelector("[data-toc-start]").value;
    const endDate=el.querySelector("[data-toc-end]").value;
    const error=el.querySelector("[data-toc-error]");
    const start=parseDay(startDate),end=parseDay(endDate);
    if(!fullName){error.textContent="Enter the learner's full name.";return}
    ...
    writeJSON(TIMELINE_KEY,{courseId:course.id,courseTitle:course.title,startDate,endDate,updatedAt:Date.now()});
    patchName();patchArch();summary();
  };
}

document.addEventListener("click",e=>{
  const toc=e.target.closest?.('[data-arch="TOC"]');
  if(!toc)return;
  e.preventDefault();
  e.stopPropagation();
  if(e.stopImmediatePropagation)e.stopImmediatePropagation();
  const t=currentTimeline();
  if(parseDay(t.startDate)===null||parseDay(t.endDate)===null)edit();else summary();
},true);
