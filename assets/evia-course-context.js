...
function timeline(){try{const x=JSON.parse(localStorage.getItem(TIMELINE_KEY)||"null");return x&&typeof x==="object"?x:{}}catch{return{}}}
function current(){
  const t=timeline();
...
function physical(key){
  const c=current();
  return c.storageSuffix&&redirected.has(String(key))?`${key}::${c.storageSuffix}`:String(key);
}
