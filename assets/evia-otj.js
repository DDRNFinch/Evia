...
function layer(body,title="Off the job",back=null){
  closeLayer();
  document.querySelector(".evia-tools-layer")?.remove();
  ...
  el.querySelector("[data-otj-back]").onclick=back||closeLayer;
  return el;
}

function main(){
  const c=college(),learner=learnerMinutes(),known=knownMinutes(),list=entries().slice().sort((a,b)=>String(b.date||"").localeCompare(String(a.date||""))||Number(b.createdAt||0)-Number(a.createdAt||0));
...
function updateCollege(){
  ...
  el.querySelector("[data-save-college]").onclick=()=>{
    ...
    const old=college();
    const snapshot={days,hours,minutes,checkedOn,savedAt:Date.now()};
...
function startRecord(existing=null){
  ...
  draft=existing?{
    id:existing.id,type:String(existing.type||""),date:String(existing.date||todayISO()),area:String(existing.area||""),topic:String(existing.topic||""),durationMinutes:entryMinutes(existing),learned:String(existing.learned||existing.description||existing.whatLearned||""),createdAt:Number(existing.createdAt)||Date.now()
...
function recordBack(step){
  if(step<=1){if(returnEntryId)viewEntry(returnEntryId);else main();return}
  renderStep(step-1);
}
...
function renderStep(step){
  ...
  if(next)next.onclick=()=>{
    if(step===4){
      draft.date=el.querySelector("[data-date]").value||todayISO();
      const h=Math.max(0,Math.round(Number(el.querySelector("[data-hours]").value)||0));
      const m=Math.max(0,Math.min(59,Math.round(Number(el.querySelector("[data-minutes]").value)||0)));
      draft.durationMinutes=h*60+m;
      if(draft.durationMinutes<=0){showMessage(el,"Add the actual learning time before continuing.");return}
    }
    renderStep(step+1);
  };
  ...
  const review=el.querySelector("[data-review]");if(review)review.onclick=()=>{draft.learned=ta.value.trim();if(!draft.learned)return;reviewEntry()};
}

function showMessage(el,text){let x=el.querySelector(".evia-otj-error");if(!x){x=document.createElement("div");x.className="evia-otj-error";el.querySelector(".evia-tools-body").appendChild(x)}x.textContent=text}

function reviewEntry(){
  const codes=topicCodes(draft.area,draft.topic);
  const el=layer(`
    <div class="evia-otj-step">Review</div><h2>Check your OTJ entry</h2>
    <p class="evia-tools-copy">Evia has only arranged the information you selected and wrote.</p>
    <div class="evia-otj-review">
      <small>${esc(draft.type)}</small><h3>${esc(draft.area)} — ${esc(draft.topic)}</h3>
      <div class="evia-otj-learned"><span>What I learned</span><p>${esc(draft.learned)}</p></div>
      <footer>${esc(formatDate(draft.date))} · ${formatMinutes(draft.durationMinutes)}</footer>
    </div>
    <button class="evia-tools-primary" data-save-entry>Save OTJ</button>
    <button class="evia-tools-secondary" data-edit-learning>Edit</button>
  `,"Review OTJ",()=>renderStep(5));
  el.querySelector("[data-save-entry]").onclick=()=>{
    const all=entries(),index=all.findIndex(e=>e.id===draft.id);
    const item={id:draft.id,type:draft.type,date:draft.date,area:draft.area,topic:draft.topic,durationMinutes:draft.durationMinutes,hours:Number((draft.durationMinutes/60).toFixed(2)),learned:draft.learned,codes,createdAt:draft.createdAt,updatedAt:Date.now()};
...
function saved(){
  const el=layer(`<div class="evia-otj-saved"><div>✓</div><h2>OTJ saved</h2><p class="evia-tools-copy">It has been added to the learner-recorded OTJ total.</p><button class="evia-tools-primary" data-back-main>Back to off the job</button></div>`,"Off the job",main);
  el.querySelector("[data-back-main]").onclick=main;
}

function viewEntry(id){
  const e=entries().find(x=>x.id===id);if(!e)return main();
  const el=layer(`
    <p class="evia-tools-kicker">Learner-recorded OTJ</p><h2>${esc(e.topic||"Off-the-job learning")}</h2>
    <p class="evia-tools-copy">${esc(e.type||"Learning")} · ${esc(e.area||"")}</p>
    <div class="evia-otj-review"><div class="evia-otj-learned"><span>What I learned</span><p>${esc(e.learned||e.description||e.whatLearned||"")}</p></div><footer>${esc(formatDate(e.date))} · ${formatMinutes(entryMinutes(e))}</footer></div>
    <button class="evia-tools-primary" data-edit-entry>Edit entry</button>
    <button class="evia-tools-secondary evia-otj-delete" data-delete-entry>Delete entry</button>
  `,"OTJ entry",main);
  el.querySelector("[data-edit-entry]").onclick=()=>startRecord(e);
  el.querySelector("[data-delete-entry]").onclick=()=>confirmDelete(e);
}
function confirmDelete(e){
  const el=layer(`<h2>Delete this OTJ entry?</h2><p class="evia-tools-copy">${esc(e.topic||"This learning record")} · ${formatMinutes(entryMinutes(e))}. This will reduce the learner-recorded OTJ total.</p><div class="evia-warning">This action cannot be undone.</div><button class="evia-tools-primary evia-otj-danger" data-confirm-delete>Delete entry</button><button class="evia-tools-secondary" data-cancel-delete>Cancel</button>`,"Delete OTJ",()=>viewEntry(e.id));
  el.querySelector("[data-cancel-delete]").onclick=()=>viewEntry(e.id);
...
document.addEventListener("click",e=>{
  ...
  if(!button)return;
  e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation();main();
},true);

const observer=new MutationObserver(patchArch);
observer.observe(document.documentElement,{subtree:true,childList:true});
window.addEventListener("load",patchArch);
window.addEventListener("pageshow",patchArch);
window.addEventListener("storage",e=>{if(e.key===ENTRY_KEY||e.key===COLLEGE_KEY)patchArch()});
