...
function decorateCodeRows(set,root=document){
  const rows=[];if(root.nodeType===1&&root.matches?.("[data-code]"))rows.push(root);root.querySelectorAll?.("[data-code]").forEach(row=>rows.push(row));
...
function decorateKsbRows(set,root=document){
  const selector=".self-ksbs button,.self-ksbs article,.self-ksbs li",rows=[];if(root.nodeType===1&&root.matches?.(selector))rows.push(root);root.querySelectorAll?.(selector).forEach(row=>rows.push(row));
  rows.forEach(row=>{const text=(row.querySelector("strong,b")?.textContent||row.textContent||"").trim(),code=normalCode(text);if(set.has(code))append(row)})
}
function decorateNvqRows(set,root=document){
  const selector=".evia-acb-ac",rows=[];if(root.nodeType===1&&root.matches?.(selector))rows.push(root);root.querySelectorAll?.(selector).forEach(row=>rows.push(row));
  rows.forEach(row=>{
    const text=(row.querySelector("b")?.textContent||row.textContent||""),unit=text.match(/\bUnit\s*(\d+)\b/i),ac=text.match(/\bAC\s*(\d+(?:\.\d+){0,3})\b/i);
    if(!ac)return;const code=(unit?`${unit[1]}.${ac[1]}`:ac[1]).toUpperCase();if(set.has(code))append(row)
  })
}
function decorateGenericRows(set,root=document){
  const selector=".evia-tools-screen .evia-tools-row",rows=[];if(root.nodeType===1&&root.matches?.(selector))rows.push(root);root.querySelectorAll?.(selector).forEach(row=>rows.push(row));
  rows.forEach(row=>{const text=(row.querySelector("b,strong")?.textContent||"").trim(),unit=text.match(/\bUnit\s*(\d+)\b/i),ac=text.match(/\bAC\s*(\d+(?:\.\d+){0,3})\b/i),direct=text.match(/^(?:AC\s*)?([KSB]?\d+(?:\.\d+){0,3}[A-Z]?)(?:\b|\s|·|:)/i);const code=unit&&ac?`${unit[1]}.${ac[1]}`:direct?.[1]||ac?.[1]||"";if(code&&set.has(code.toUpperCase()))append(row)})
}
...
function summary(set){
  document.querySelectorAll(".evia-acb-layer,.evia-nvq-layer").forEach(layer=>{
    let line=layer.querySelector(".evia-milos-observed-summary-v94");if(!set.size){line?.remove();return}
...
function patch(root=document){queued=false;clearHome();const set=observedSet();if(!set.size){summary(set);return}decorateCodeRows(set,root);decorateKsbRows(set,root);decorateNvqRows(set,root);decorateGenericRows(set,root);summary(set)}
...
function hookObservedStorage(){
  if(window.__eviaMilosObservedStorageHookV94)return;window.__eviaMilosObservedStorageHookV94=true;const native=Storage.prototype.setItem;
...
function start(){
  hookObservedStorage();patch(document);const root=document.getElementById("root")||document.body;
