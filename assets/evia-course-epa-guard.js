...
function ensureStyles(){
  ...
  s.textContent=`
.${BACKDROP_CLASS}{position:fixed;inset:0;z-index:100005;background:linear-gradient(180deg,#fcfcfd 0%,#f8f8fb 72%,#f1f2f8 100%);opacity:1;pointer-events:auto;transition:opacity .42s ease}
.evia-course-epa-layer{z-index:100010!important}
.evia-course-epa .evia-tools-body{padding-top:6.95rem!important}
.${AVATAR_CLASS}{position:fixed;z-index:100020;top:calc(env(safe-area-inset-top,0px) + 3.35rem);left:50%;width:6rem;height:6rem;opacity:0;pointer-events:none;transform:translate(-50%,8px) scale(.965);transition:opacity .46s ease,transform .62s cubic-bezier(.22,1,.36,1)}
